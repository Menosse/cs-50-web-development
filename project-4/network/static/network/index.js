...
document.addEventListener('DOMContentLoaded', function(){
    ...
    document.querySelector('#user-page').addEventListener('click', () => load_posts(localStorage.getItem('user')));
...
function get_user(){
    ...
    .then(result =>{localStorage.setItem('user', result.user)})
}
...
function load_posts(postkind,page){
    ...
    if(postkind==='all'){
        ...
        document.querySelector('#post-body').style.display = 'block'
        document.querySelector("#user-details-view").style.display = 'None'
    }if(postkind === 'following'){
        document.querySelector('#compose-view').style.display = 'None'
        document.querySelector('#posts-view').style.display = 'block'
        document.querySelector('#footer').style.display = 'None'
        document.querySelector("#user-details-view").style.display = 'None'
    }if(postkind === localStorage.getItem('user')){
        document.querySelector('#compose-view').style.display = 'block'
        document.querySelector('#posts-view').style.display = 'block'
        document.querySelector('#footer').style.display = 'None'
        document.querySelector("#user-details-view").style.display = 'block'
        profile(localStorage.getItem('user'))
        postkind = localStorage.getItem('user')
    }else{
        //document.querySelector('#compose-view').style.display = 'None'
        document.querySelector('#posts-view').style.display = 'block'
        //document.querySelector('#footer').style.display = 'None'
...
function get_posts(postkind, page){
    ...
    .then(result =>{
        result.posts.forEach(post => {
            ...
            user_link.addEventListener('click', () => load_posts(post.user));
            user_link.href = '#'
...
function config_like_button(){
    document.querySelectorAll(".like-button").forEach(button => {
        button.onclick = () =>{
            like_post(button.id)
            console.log(`button done - ${button.id}`)
        }
...
function profile(user){
    ...
    .then(result =>{
        ...
        const user_detail_info = document.createElement('h3');
        user_detail_info.innerHTML = `Following: ${result.following_number} Followers: ${result.followers_number}`
        document.querySelector("#user-details-view").appendChild(user_detail_info)
    })
    .catch(error =>{
        console.log("Error", error)
    })
